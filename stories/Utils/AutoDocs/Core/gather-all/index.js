...
const gatherAll = path =>
  isPath(path)
    ...
    .then(files => {
      const metadata = metadataParser(path).catch(e =>
      ...
      const readmeTest = readMarkdown('readme.test.md');
      ...
      return Promise.all([
        ...
        readmeAccessibility,
        readmeTest,
      ]).then(([metadata, readme, readmeAccessibility, readmeTest]) => {
        metadata.componentPath = path;
        return {
          ...metadata,
          readme,
          readmeAccessibility,
